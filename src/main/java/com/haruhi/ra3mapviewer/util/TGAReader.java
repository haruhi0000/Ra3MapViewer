...
public final class TGAReader {
    ...
    public static Image convertToFxImage(File path) throws TgaReaderException {
        ...
        try (FileInputStream fis = new FileInputStream(path)) {
            ...
            return TGAReader.convertToFxImage(image2);
        } catch (IOException | ArrayIndexOutOfBoundsException e) {
            throw new TgaReaderException(e.getMessage());
        }


    }

    public static Image convertToFxImage(byte[] bytes) throws  TgaReaderException {
    ...
    public static int[] read(byte[] buffer, Order order) throws TgaReaderException {
        ...
        switch (type) {
            ...
            break;
            default:
                throw new TgaReaderException("Unsupported image type: " + type);
        }
            ...
            if ((packet & 0x80) != 0) { // RLE
                ...
                for (int i = 0; i < count; i++) {
                    for (int j = 0; j < elementCount; j++) {
                        if(decoded >= decodeBuffer.length-1) {
                            break;
                        }
                ...
                for (int i = 0; i < count; i++) {
                    ...
                    if(decoded >= decodeBuffer.length || offset >= buffer.length) {
                        break;
                    }
                }
            }
    ...
    private static int[] createPixelsFromColormap(int width, int height, int depth, byte[] bytes, int offset,
                                                  byte[] palette, int colormapOrigin, int descriptor, Order order) throws TgaReaderException {
        int[] pixels;
        ...
        switch (depth) {
            ...
            case 32:
                ...
                break;
            default:
                throw new TgaReaderException("Unsupported depth:" + depth);
        }
        return pixels;
    }

    private static int[] createPixelsFromRGB(int width, int height, int depth, byte[] bytes, int offset,
                                             int descriptor, Order order) throws TgaReaderException {
        int[] pixels;
        int rs = order.redShift;
        int gs = order.greenShift;
        int bs = order.blueShift;
